           ...
           //var trialIsRunning = 0;
            ...
            console.log('indexE.html is creating socket');
            ...
            socket.on('serverUpdate', function(msg) {
                //console.log('received serverUpdate');
                	...
                	$('#currentSessionNumberID').empty().append('Session : ' + json.sessionNumber);
                	
                	numTrialStr = parseInt(json.numTrial) - 1
                	$('#currentTrialNumberID').empty().append('Trial : ' + json.trialNumber + '/' + numTrialStr);
                	$('#currentDurID').empty().append('Duration (ms): ' + json.sessionDur);
			...
			$('form#trialform').submit(function(event) {
				trialFormDict = {
					numTrial: $('#numTrialID').val(),
					trialDur: $('#trialDurID').val(),

					interTrialIntervalLow: $('#interTrialIntervalLowID').val(),
					interTrialIntervalHigh: $('#interTrialIntervalHighID').val(),
					preCSdur: $('#preCSdurID').val(),
					CSdur: $('#CSdurID').val(),
					USdur: $('#USdurID').val(),
					percentUS: $('#percentUSID').val(),
					percentCS: $('#percentCSID').val(),
				...
				console.log('indexE.html is sending trialform:' + trialFormDict);
            ...
            events: {
                load: function() {
                    ...
                    socket.on('serialdata', function (msg) {
						...
						//3
						} else if (event == "ledCSon") {
                          	millis = millis - trialStartMillis;
	                       	CSstartMillis = millis;
	                       	//var series3 = chart.series[2];
                        	//series3.addPoint([millis, 3]);
						} else if (event == "ledCSoff") {
                         	millis = millis - trialStartMillis;
	                       	CSstopMillis = millis;
                        	//var series3 = chart.series[2];
