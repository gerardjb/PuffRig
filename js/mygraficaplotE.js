var mygraficaplotE = function(p) {
	...
	p.setup = function() {
		...
		plot.setPointColor(dataColor);
			...
			plot.addLayer("csLayer", pointsCS);
			plot.getLayer("csLayer").setPointColor(p.color(10, 10, 255));
		plot.setYLim(0, 255);
	...
	p.myupdate = function(serialData) {
		...
		} else if ((event == "ledCSon") || (event == "ledCSoff")) {
			pnt = new GPoint(res[0], 100);
			pointsCS.push(pnt);
			plot.setXLim(xMin,xMax); //last 10 seconds
			plot.getLayer("csLayer").setPoints(pointsCS);
